...
import type { Catalog, Plugin } from './schema.js'
import {
  byteDigest,
  createArtifacts,
  digest,
...
describe('artifact shards', () => {
  ...
  test('advertises an absolute immutable URL when the publication is known', () => {
    ...
    expect(index.products.calendar!.icon!.assetUrl).toBe(
      `https://raw.githubusercontent.com/adea-ai/plugins/catalog-assets/catalogs/${base.catalogId.slice('catalog:'.length)}/${svgAsset}`
    )
  ...
  test('serves marks and the index from the committed asset base when configured', () => {
    ...
    expect(index.products.calendar.icon.assetUrl).not.toContain('/releases/')
  })
  ...
  test('carries compiled marks in the navigation artifact', () => {
    ...
    expect(navigation.catalogIndexUrl).toBe(
      `https://raw.githubusercontent.com/adea-ai/plugins/catalog-assets/catalogs/${base.catalogId.slice('catalog:'.length)}/catalog-index.v1.json`
    )
    expect(Object.keys(navigation.brandMarks)).toEqual(['calendar'])
    expect(navigation.brandMarks.calendar).toContain(
      `/catalogs/${base.catalogId.slice(8)}/${svgAsset}`
    )
