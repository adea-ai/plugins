...
export interface ConsumerIndexOptions {
  ...
  /**
   * Base URL of the content-addressed snapshot directory, one level below the
   * asset branch. Catalog artifacts are addressed by catalog identity, so they
   * live in `catalogs/<catalogId>/` while digest-addressed brand marks sit at
   * the branch root.
   */
  readonly publicationCatalogsBaseUrl?: string
  /**
...
export async function synchronize(input: SyncInput): Promise<SyncResult> {
  ...
  const artifacts = createArtifacts(orderLeadingPlugins(catalog, input.leading), lock, {
    ...
    ...(input.publicationAssetsBaseUrl
      ...
      : {}),
    ...(input.publicationCatalogsBaseUrl
      ? { publicationCatalogsBaseUrl: input.publicationCatalogsBaseUrl }
      : {}),
...
export function createArtifacts(
  ...
  const index = buildConsumerIndex({
    ...
    ...(options.publicationAssetsBaseUrl
      ...
      : {}),
    ...(options.publicationCatalogsBaseUrl
      ? { publicationCatalogsBaseUrl: options.publicationCatalogsBaseUrl }
      : {}),
  })
  const summary = {
    schemaVersion: 1,
  ...
  const navigation = {
    ...
    ...(options.publicationCatalogsBaseUrl
      ? {
          catalogIndexUrl: `${options.publicationCatalogsBaseUrl}/${catalog.catalogId.slice('catalog:'.length)}/${CATALOG_INDEX_ARTIFACT}`,
        }
      : options.publicationAssetsBaseUrl
